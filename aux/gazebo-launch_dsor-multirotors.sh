...
function spawn_model() {
	...
	python3 ${src_path}/Tools/sitl_gazebo/scripts/jinja_gen.py ${src_path}/Tools/sitl_gazebo/models/${MODEL}/${MODEL}.sdf.jinja ${src_path}/Tools/sitl_gazebo --mavlink_tcp_port $((4560+${N})) --mavlink_udp_port $((14540+${N})) --mavlink_id $((1+${N})) --gst_udp_port $((5600+${N})) --video_uri $((5600+${N})) --mavlink_cam_udp_port $((14530+${N})) --output-file /tmp/${MODEL}_${N}.sdf
...
px4_conf_file_dir=${src_path}/ROMFS/px4fmu_common/init.d-posix
if [ ! -f "${px4_conf_file_dir}/px4-rc.mavlink.template" ]; then
  ...
  sed 's/udp_gcs_port_local=$((18570+px4_instance))/udp_gcs_port_local=$((18570+px4_instance))\ntarget_ip=TEMPLATE_IP/g' ${px4_conf_file_dir}/px4-rc.mavlink > ${px4_conf_file_dir}/px4-rc.mavlink.template
...
sed "s/TEMPLATE_IP/${target_ip}/g" ${px4_conf_file_dir}/px4-rc.mavlink.template > ${px4_conf_file_dir}/px4-rc.mavlink
echo $target_ip
